...
- `inference_server.py` — IMPALA-style GPU inference server: dynamic-batches encoded text-rollout requests from N actor threads into one forward pass, stages the merged replay payload, scatters per-request host scalars back via futures. Pause/resume API for PPO update windows.
