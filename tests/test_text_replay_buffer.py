...
class TextReplayBufferTests(unittest.TestCase):
    ...
    def test_release_rejects_non_fifo_rows(self) -> None:
        ...
        self.assertEqual(snapshot["claimed_windows"], 1)
        self.assertEqual(snapshot["claimed_start"], 0)
        self.assertEqual(snapshot["claimed_limit"], 2)
    ...
    def test_release_rejects_claimed_window_away_from_ring_head(self) -> None:
        buffer = TextReplayBuffer(
            capacity=3,
            max_tokens=2,
            max_options=2,
        ...
        buffer.row_ring.start = 1
        with self.assertRaisesRegex(RuntimeError, "row ring head"):
            buffer.release_train_window(window)
